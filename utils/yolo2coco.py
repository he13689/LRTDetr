...
parser.add_argument('--root_dir', default='algae_oa', type=str, help="root path of images and labels, include ./images and ./labels and classes.txt")
parser.add_argument('--save_path', type=str, default='./train.json', help="if not split the dataset, give a path to a json file")
parser.add_argument('--random_split', default=True, help="random split the dataset, default ratio is 8:1:1")
...
def train_test_val_split_random(img_paths, ratio_train=0.8, ratio_test=0.1, ratio_val=0.1):
    ...
    train_img, middle_img = train_test_split(img_paths, train_size=ratio_train, test_size=1 - ratio_train, random_state=233)
    ratio = ratio_val / (1 - ratio_train)
    val_img, test_img = train_test_split(middle_img, train_size=ratio, test_size=1-ratio, random_state=233)
...
def yolo2coco(arg):
    ...
    with open(os.path.join(root_path, 'classes.txt'), encoding='utf-8') as f:
    ...
    if arg.random_split or arg.split_by_file:
        ...
        # 建立类别标签和数字id的对应关系, 类别id从0开始。 但是在RTDETR中，也就是coco2017中，id应当从1开始
        ...
        if arg.random_split:
            ...
            train_img, val_img, test_img = train_test_val_split_random(indexes, 0.9, 0.01, 0.09)
    ...
    for k, index in enumerate(tqdm(indexes)):
        ...
        with open(os.path.join(originLabelsDir, txtFile), 'r') as fr:
            ...
            for label in labelList:
                ...
                if root_path == 'data2' and int(label[0]) == 16:
                    continue
                x = float(label[1])
                y = float(label[2])
                w = float(label[3])
                ...
                cls_id = int(label[0]) + 1  # coco 2017 数据集是从1开始的
                width = max(0, x2 - x1)
                height = max(0, y2 - y1)
                area = width * height
                if area >= 5000:
